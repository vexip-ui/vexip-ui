<template>
  <div>
    <Button type="warning" @click="$message.warning({ content: '一条自定义图标的消息', icon: 'question-circle' })">
      自定义图标
    </Button>
    <Button type="error" @click="$message.error({ content: '一条自定义图标颜色的消息', iconColor: 'green' })">
      自定义图标颜色
    </Button>
    <Button type="primary" @click="$message.info({ content: '一条对象渲染图标的消息', icon: { name: 'spinner', pulse: true } })">
      对象渲染图标
    </Button>
